...
checkpoint_path = "CNN2D_Residual\\cp_dropout.ckpt.keras"
load = False
if load:
    ...
    x = Conv2D(64, 7, padding='same')(input_layer)
    x = Dropout(.2)(x)
    #x = residual_block(x, 64, 5)
    x = residual_block(x, 64, 3)
    x = Flatten()(x)
    output_layer = Dense(train_labels_enc.shape[1], activation='softmax')(x)
